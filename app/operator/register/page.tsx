...
export default function OperatorRegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err && typeof err === 'object' && 'status' in err && err.status === 422 && 'validationErrors' in err) {
  ...
  // Don't render the registration form if not in demo mode (will redirect)
  if (!isDemoMode) {
    return null;
  }
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <CardFooter className="flex flex-col items-center gap-4 border-t border-gray-100 bg-gray-50/50 pt-6">
            <div className="text-center text-sm text-gray-500">
              ...
              <Link href="/operator/login?demo=true" className="font-semibold text-[var(--veristiq-primary-blue)] hover:underline transition-all">
                Sign in
              </Link>
            </div>
